...
export const useGame = defineStore('game', {
  ...
  getters: {
    ...
    getOpponent(): Player | null {
      return this.getPlayers?.find((p) => p?.side !== this.getOurSide) || null;
    }
  },
  actions: {
    async create(timePerMove: number = 60) {
      try {
        const { user } = useAuth();
        if (!user) throw new Error('Not authenticated yet trying to create chess room');
        const room = await ChessService.createChessRoom(timePerMove);
    ...
    async setGameOver() {
      try {
        ...
        const info = getGameOverInfo(
          this.currGame.board as Board,
          this.currGame.id,
          this.currGame.players as [Player, Player]
        );
        if (info) {
    ...
    async move(board: Board) {
      try {
        ...
        const gameStatusAfterMove =
          this.currGame.status === GameStatus.NOT_STARTED ? GameStatus.PROCESS : this.currGame.status;
        const movingSideAfterMove = this.currGame.movingSide === 'white' ? 'black' : 'white';
        await ChessService.updateChessRoom(this.currGame.id, {
          board,
          status: gameStatusAfterMove,
          movingSide: movingSideAfterMove
        } as ChessRoom);
      } catch (e) {
        console.error(e);
      }
